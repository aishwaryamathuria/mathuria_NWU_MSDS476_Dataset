...
for i in range(1,4):
    for j in range(1,11):
        folder_name = 'robot_baristas'
        if not os.path.exists(folder_name):
            os.makedirs(folder_name)
        baristas = i  # number of barista server resources... vary from 1 to 3

        minimum_service_time = 1 # minutes (60 seconds)
        mean_service_time = 1.5 # minutes (120 seconds)
        maximum_service_time = 3 # minutes (300 seconds)

        mean_inter_arrival_time = j # vary from 1 to 10 minutes (60 to 600 seconds)
        ...
        with open(os.path.join(folder_name, simulation_trace_file_name), 'wt') as ftrace:
            for d in data:
        ...
        event_log_df = pd.DataFrame({
                                    'barista_count_setting': 'robot_' + str(baristas),
                                    'interarrival_setting': mean_inter_arrival_time,
                                    'caseid':caseid_list,
                                    'time':time_list,
        ...
        event_log_df.to_csv(os.path.join(folder_name, event_log_file_name), index = False)  
        print()
        ...
        with open(os.path.join(folder_name, case_by_case_event_file_name), 'wt') as fcasedata:
        ...
        # output_string_key.append("barista_count_setting")
        # output_string_value.append(baristas)
        # output_string_key.append("interarrival_setting")
        # output_string_value.append(mean_inter_arrival_time)
        ...
        output_data.append({
            "barista_count_setting": 'robot_' + str(baristas),
            "interarrival_setting": mean_inter_arrival_time,
            "Minimum inter-arrival_time": str(round(np.min(inter_arrival_times), 2)),
            "Average inter-arrival_time": str(round(np.average(inter_arrival_times), 2)),
            "Maximum inter-arrival_time": str(round(np.max(inter_arrival_times), 2)),
            "Minimum waiting time": str(round(np.min(waiting_time), 2)),
            "Average waiting time": str(round(np.average(waiting_time), 2)),
            "Maximum waiting time": str(round(np.max(waiting_time), 2)),
            "Minimum service time": str(round(np.min(service_time), 2)),
            "Average service time": str(round(np.average(service_time), 2)),
            "Maximum service time": str(round(np.max(service_time), 2))
        })
